...
const CHECKER_ORG_WIDTH = 108;
const CHECKER_ORG_HEIGHT = 108;

const WHITE_CHECKER = 0;
const BLACK_CHECKER = 1;
var MY_TYPE = BLACK_CHECKER;
var OTHER_TYPE = WHITE_CHECKER;
const MY_DICE_POS = true; // true - right, false - left

const CHECKER_WIDTH = 57;
const CHECKER_HEIGHT = 57;
const GAP_X = 69;

const TARGET_BOARD = 0;
const TARGET_TRASH = 1;
const TARGET_BOX = 2;
...
var getLocation = function(type, row, n, m=-1) {
    if (row === ROW_TRASH)
        return getLocationOnTrash(type, n, m);
    if (row === ROW_BOX)
        return getLocationOnBox(type, n, m);
    ...
    let sy = 0;
    let dx = 0;
    let dy = (n < 5) ? CHECKER_HEIGHT : (CHECKER_HEIGHT*4)/n;
...
var getLocationOnTrash = function(type, n, m=-1) {
    let sx = 519.5;
    let sy = (type === MY_TYPE) ? 217.5 : 581.5;
    let sign = (type === MY_TYPE) ? 1 : -1;
    let dy = 10;
    return {
        x: sx,
        y: sy + sign * dy * (m>-1?m:n)
    }
}

var getLocationOnBox = function(type, n, m=-1) {
    let sx = 1109;
    let sy = (type === MY_TYPE) ? 748.5 : 55.5;
    let sign = (type === MY_TYPE) ? -1 : 1;
    let dy = 13;
    return {
        x: sx,
